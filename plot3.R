...
# Select Date and sub_metering 1-3 ; subset dates: include only 2007-02-01 and 2007-02-02
powerConsumptionSub.dat <- powerConsumption.dat %>%
  dplyr::select(Date, Time, Sub_metering_1, Sub_metering_2, Sub_metering_3) %>%
...
#--------------------------------------------------Plot 3-------------------------------------------------------#
# Time series of Energy sub metering, save to file
png("plot3.png", width=480, height=480) # Open connection ; specify path/file name and image dimensions
plot(powerConsumptionSub.dat$Date2, powerConsumptionSub.dat$Sub_metering_1, type = "l", xlab = "", ylab = "Energy sub metering")
lines(powerConsumptionSub.dat$Date2, powerConsumptionSub.dat$Sub_metering_2, type = "l", col = "red")
lines(powerConsumptionSub.dat$Date2, powerConsumptionSub.dat$Sub_metering_3, type = "l", col = "blue")
